...
async function fetchProductPageWithBrowser(url) {
  ...
  if (!playwright?.chromium) {
    throw new Error("BROWSER_FALLBACK_UNAVAILABLE");
  }
  ...
  try {
    ...
    await page.goto(normalizeProductUrl(url), { waitUntil: "domcontentloaded", timeout: 30000 });
...
async function extractFromUrl(url) {
  try {
    ...
    if (!primaryError || ![403, 429].includes(primaryError.status)) {
      throw primaryError;
    }
    try {
      const { html, finalUrl } = await fetchProductPageWithBrowser(url);
      return extractProductData(html, finalUrl);
    } catch (browserError) {
      const error = new Error(`${primaryError.message};BROWSER_FALLBACK_FAILED:${browserError.message}`);
