...
function parseJsonLikeBlocks(html) {
  ...
  for (const match of scriptMatches) {
    ...
    const candidates = [
      raw,
      ...(raw.match(/(?:window\.__[^=]+|__NEXT_DATA__|__PRELOADED_STATE__)\s*=\s*({[\s\S]*?})\s*;?/g) || []).map((entry) =>
        entry.replace(/^[^=]+=\s*/, "").replace(/;$/, "")
      ),
    ];
...
function extractCategory(html, objects) {
  ...
  return pickFirstMeaningful([breadcrumb, objectValue, breadcrumbMatch?.[1] || ""]);
}
...
function extractProductData(html, sourceUrl) {
  ...
  const objects = flattenObjects([...jsonLdNodes, ...jsonBlocks]);
  ...
  const category = extractCategory(html, objects);
  ...
  const priceCandidates = [
    offers?.price,
    offers?.lowPrice,
    extractMeta(html, ["product:price:amount", "og:price:amount", "twitter:data1", "price"]),
    findValueInObjects(objects, [
      "salePrice",
      "sellPrice",
      "discountedPrice",
    ...
    ...findAllMatches(html, /(?:\"|\')(?:salePrice|sellPrice|finalPrice|productPrice|originPrice|price)(?:\"|\')\s*:\s*(?:\"|\')?([0-9][0-9,]{2,}(?:\.[0-9]+)?)/gi),
    ...findAllMatches(html, /(판매가|할인가|가격|정가)[^0-9]{0,20}([0-9][0-9,]{2,}(?:\.[0-9]+)?)/gi),
  ];
  ...
  return {
    ...
    sku,
    image,
    price: normalizePrice(pickFirstMeaningful(priceCandidates)),
    currency,
    availability,
