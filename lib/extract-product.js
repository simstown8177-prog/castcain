...
function extractCategory(html, objects) {
  ...
  if (raw) return raw.split(/[>,/|]/).map((part) => cleanText(part)).filter(Boolean).pop() || raw;

  const fullText = stripTags(html).toLowerCase();
  const categoryRules = [
    ["육류", ["소고기", "돼지고기", "닭", "오리", "목살", "삼겹", "갈비", "정육", "축산"]],
...
function scorePriceCandidate(value, sourceLabel, context = "") {
  ...
  if (/판매가|할인가|최종가|sale|final/i.test(context)) score += 3;
  if (/정가|소비자가|list/i.test(context)) score -= 1;
